...
if image_exists "agentic-research-agentic-research" \
  ...
  docker compose -f docker-compose.yml -f docker-compose.dgx-vllm-gptoss120b-mono.yml --env-file "$ENV_FILE" build \
    --build-arg APP_VERSION="${APP_VERSION}" \
    dataprep agentic-research embeddings-gpu
fi

docker compose -f docker-compose.yml -f docker-compose.dgx-vllm-gptoss120b-mono.yml --env-file "$ENV_FILE" up -d \
  chromadb dataprep embeddings-gpu llm

echo "Services started. Run research with:"
echo "docker compose -f docker-compose.yml -f docker-compose.dgx-vllm-gptoss120b-mono.yml --env-file $ENV_FILE run --rm agentic-research agentic-research --config /app/configs/config-docker-dgx-vllm-gptoss120b-mono.yaml --query 'your query'"
