# Agentic Research — Sequence Diagrams

## Full Flow (Simplified — 5 participants)

```mermaid
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '16px', 'actorTextColor': '#000', 'signalTextColor': '#333', 'noteBkgColor': '#fff3bf', 'noteTextColor': '#000'}}}%%
sequenceDiagram
    ...
    Note over M,F: Phase 1 — Knowledge Preparation
    M->>L: knowledge prep prompt
    L->>D: get_knowledge_entries()
    D-->>L: existing entries
    L->>D: download_and_store_url(url)
    D->>D: HTTP fetch + parse HTML to MD
    D->>L: extract keywords + summary
    L-->>D: metadata
    D->>V: upload chunks (auto-embeds)
    V-->>D: indexed
    D-->>L: agenda

    Note over M,F: Phase 2 — Planning
    M->>L: plan search queries
    L-->>M: FileSearchPlan (N queries)

    Note over M,F: Phase 3 — Parallel Search
    M->>L: search query (x N parallel)
    L->>V: vector_search(query)
    V-->>L: ranked chunks
    L->>F: write_file(summary.txt)

    Note over M,F: Phase 4 — Writing
    M->>L: write report prompt
    L->>F: read_multiple_files(results)
    F-->>L: search summaries
    L-->>M: ReportData (markdown + summary)
```

## Detailed Flow (15 participants)
...
sequenceDiagram
    ...
    Note over KPAgent: StopAtTools triggers
