...
def build_fs_server_params(
    temp_dir: str, output_dir: str | None = None
...
def _is_function_call_successful(messages: list[dict[str, Any]], call_id: str) -> bool:
    ...
    """
    for msg in messages:
        if msg.get("type") == "function_call_output" and msg.get("call_id") == call_id:
...
def save_result_input_list_to_json(
    model_name: str, report_file_name: str, messages: list, output_report_dir: str
) -> str:
    """
...
def save_trajectory_evaluation_report(
    model_name: str, output_report_dir: str, report_file_name: str, human_readable_report: str
) -> str:
    """
    ...
    base_file_name = os.path.basename(report_file_name).rsplit(".md", 1)[0]
    safe_model_name = model_name.replace("/", "-")
    trajectory_evaluation_file_name = (
        f"{base_file_name}_{safe_model_name}_trajectory_evaluation.txt"
    )
...
def validate_trajectory_spec(
    messages: list[dict[str, Any]], spec: list[dict[str, Any]]
) -> dict[str, Any]:
    """
    ...
    for i, m in enumerate(messages):
        if m.get("type") == "function_call":
            ...
            if content.strip():
                events.append({"type": "generation", "content": content, "msg": m, "index": i})
        ...
        for idx in range(pos, len(events)):
            ...
            if step["type"] == "function_call":
                ...
                if (
                    ...
                    and ev.get("successful", False)  # ✅ NOUVEAU : Vérifier le succès
                    and (
                        ev["name"] == step.get("name")
                        or (step.get("name_prefix") and ev["name"].startswith(step["name_prefix"]))
                    )
                ):
                    matched = True
                    matched_event = ev
                    ...
                    if arguments:
                        try:
                            ...
                            if markdown_content:
                                ...
                                decoded_content = _decode_json_escapes(markdown_content)
                                match = re.search(
                                    step["match_regex"], decoded_content, re.MULTILINE
                                )
                                if match:
                                    matched = True
                                    matched_event = ev
            ...
            results.append(
                {
                    ...
                    "required": step.get("required", True),
                    "found": False,
                    "status": "MANQUANT (REQUIS)"
                    if step.get("required", True)
                    else "MANQUANT (OPTIONNEL)",
                    "found_text": reason,
...
def format_trajectory_report(
    model_name: str, evaluation: dict[str, Any], title: str = "Agent Trajectory"
) -> str:
    """
    ...
    if not evaluation["success"]:
        ...
        report.append(
            "• Vérifier les patterns regex et noms de fonctions dans TRAJECTORY_SPEC_MODEL"
        )
        report.append(
            "• Examiner les messages générés pour s'assurer qu'ils contiennent les patterns attendus"
        )
