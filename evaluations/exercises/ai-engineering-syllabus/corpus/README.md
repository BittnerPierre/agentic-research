...
`data/` directory. New runs persist the exact retrieved passages in `chunks.json`.
`sources.json` citations must resolve to those passages, and each passage must validate
against a SHA-pinned raw file. A generated search summary with no resolved raw chunk is
